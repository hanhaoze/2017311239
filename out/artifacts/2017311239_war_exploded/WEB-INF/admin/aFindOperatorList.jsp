            ...
            <form style="padding-top: 20px;" action="${pageContext.request.contextPath}/findOperatorByPageServlet" method="post">
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <label class="layui-form-label" style="width: auto;">员工号</label>
                        ...
                        <div class="layui-input-inline">
                            ...
                            <a class="layui-btn layui-btn-danger" href="javascript:void(0);" id="deleteSelectOperator">删除选中</a>
                        </div>
                    </div>
                </div>
            </form>

            <form id="selectOperator" action="${pageContext.request.contextPath}/deleteSelectOperatorServlet" method="post">
                <table class="layui-table" lay-filter="test">
                    ...
                    <c:forEach items="${pb.list}" var="operator" varStatus="s">
                        <tr>
                            <th><input type="checkbox" class="my-checkbox" name="sid" value="${operator.s_id}" title="" lay-skin="primary"></th>
                            <td>${s.count}</td>
                            <td>${operator.s_id}</td>
                            ...
                            <td><a class="layui-btn layui-btn-normal" href="${pageContext.request.contextPath}/updateOperatorServlet?sid=${operator.s_id}">修改</a><!--<a class="layui-btn layui-btn-danger" href="javascript:deleteOperator(${operator.s_id});">删除</a>---></td>
                        </tr>
                    </c:forEach>
                </table>
            </form>
...
<script>
    function deleteOperator(sid) {
        if (confirm("你确定删除该学生吗？")) {
            location.href = "${pageContext.request.contextPath}/deleteOperatorServlet?sid=" + sid;
        }
    }

    window.onload = function () {
        document.getElementById("deleteSelectOperator").onclick = function () {
            var flag = false;
            ...
            if (flag) {
                if (confirm("你确定要删除选中条目吗？")) {
                    document.getElementById("selectOperator").submit();
                }
            }
